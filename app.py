...
def app():
    ...
    image = Image.open('coventrysearchlogo.png')
    st.image(image)

    # Add a text description
    st.markdown("<p style='text-align: center;'> The Coventry University Centre for Intelligent Healthcare(RCIH) and other related research paper,journal and researcher profile finder.</p>", unsafe_allow_html=True)


    input_text = st.text_input("Search research or researcher profile:", key="query_input")
    ...
    if st.button("SEARCH"):
        if search_type == "Publications":
            output_data = search_data(input_text, 1 if operator_val == 'Same' else 2, "publication")
        elif search_type == "Authors":
            output_data = search_data(input_text, 1 if operator_val == 'Same' else 2, "author")
        else:
            output_data = {}
    ...
    st.markdown("<p style='text-align: center;'><a>STW7071CEM-INFORMATION RETRIEVAL(Search Engine Project)</a></p> <a href='https://github.com/shumshersubashgautam'>Subash Gautam</a> <p>Softwarica College Id:230083 </p> <p>Msc-Data Science & Computational Intelligence</p> © Coventry University Id:14224650</p>", unsafe_allow_html=True)
